...
public class ZWSignCalendarView extends FrameLayout {
    ...
    private void init(AttributeSet attrs, int def) {
        ...
        if (config.isShowLunar) {
            ...
            config.lunarTextSize = array.getDimension(R.styleable.ZWSignCalendarView_lunarTextSize, sp2px(0));
        }
        ...
        config.signTextColor1 = array.getColor(R.styleable.ZWSignCalendarView_signTextColor, Color.parseColor("#333333"));
        ...
        config.signColor1 = array.getColor(R.styleable.ZWSignCalendarView_weekBackgroundColor, Color.GRAY);
        array.recycle();
        initPager();
    }
     ...
     * @param signRecords 为日期格式像 "2017-08-25"
     */
    public void setSignRecords1(final HashSet<String> signRecords) {
        if (signRecords == null) return;
        this.signRecords = signRecords;
        for (ZWSignCalendar view : viewSet) {
            view.setSignRecords1(signRecords);
            view.invalidate();
        }
    }
